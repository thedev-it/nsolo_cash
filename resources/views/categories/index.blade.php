<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto space-y-6 md:space-y-10 px-4 sm:px-0 py-6">
        @php
        ...
        @foreach ([
            ...
            <div class="rounded-2xl border border-gray-100 overflow-hidden shadow-sm bg-white">
                ...
                {{-- Bandeau de section --}}
                <div class="flex items-center justify-between px-4 sm:px-6 py-4" style="background-color: {{ $group['tint'] }}">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="font-semibold text-gray-900 leading-tight text-sm sm:text-base">{{ $group['label'] }}</h3>
                            <p class="text-xs text-gray-500">{{ $group['items']->count() }} catégorie{{ $group['items']->count() > 1 ? 's' : '' }}</p>
                        </div>
                    </div>
                    <a href="{{ route('categories.create') }}"
                       class="text-xs font-semibold px-3 py-1.5 rounded-full bg-white/70 hover:bg-white transition shrink-0"
                ...
                <div class="p-4 sm:p-6 bg-white">
                    ...
                    @else
                        {{-- Grille adaptative : 1 colonne sur mobile, 2 colonnes à partir de 480px, 3+ sur desktop --}}
                        <div class="grid grid-cols-1 [@media(min-width:480px)]:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                            ...
                            <div class="group relative rounded-2xl border border-gray-100 p-5 pt-6 bg-white hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 overflow-hidden">
                                ...
                                {{-- Halo coloré diffus en fond --}}
                                <div class="absolute -top-8 -right-8 w-24 h-24 rounded-full opacity-[0.08] pointer-events-none"
                                     style="background-color: {{ $category->color }}"></div>

                                {{-- Actions tactiles et survol (Toujours visible sur mobile, masqué par défaut sur desktop avant survol) --}}
                                <div class="absolute top-3 right-3 flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('categories.edit', $category) }}"
                                       class="w-7 h-7 flex items-center justify-center rounded-full bg-gray-50 text-gray-500 hover:bg-indigo-50 hover:text-indigo-600 border border-gray-100 md:border-0 shadow-sm md:shadow-none">
                                    ...
                                    <form action="{{ route('categories.destroy', $category) }}" method="POST"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                                class="w-7 h-7 flex items-center justify-center rounded-full bg-gray-50 text-gray-500 hover:bg-red-50 hover:text-red-500 border border-gray-100 md:border-0 shadow-sm md:shadow-none">
                                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                ...
                                <p class="font-semibold text-gray-900 truncate relative z-10 pr-12 md:pr-0">{{ $category->name }}</p>
