<div x-data="{ openMobileMenu: false }">
    ...
    <aside :class="openMobileMenu ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
        ...
        <div class="border-t border-gray-100 p-3 space-y-1">
            <a href="{{ route('profile.edit') }}"class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium no-underline
                    {{ request()->routeIs('profile.*') ? 'bg-indigo-50 text-indigo-600' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
